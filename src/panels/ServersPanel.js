                ...
                <ul>
                    {
                        servers.map((server, index) => (
                            <ServerItem server={server} key={index} {...{ editServer, deleteServer }} />
                        ))
                    }
                </ul>
            </div>
...
function ServerItem({ server, id, deleteServer, editServer }) {
    ...
    return (
        <li className="flex items-center justify-between hover:bg-indigo-400 py-2 px-2 rounded">
            ...
            <div className="flex space-x-2">
                <svg onClick={() => editServer(server)} className="w-4 h-4 cursor-pointer transition hover:text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
